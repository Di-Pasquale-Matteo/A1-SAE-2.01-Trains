...
public class Jeu implements Runnable {
    ...
    public Jeu(String[] nomsJoueurs, String[] cartesPreparation, Plateau plateau) {
        ...
        this.cartesEnJeu = new ListeDeCartes();
    }
    ...
    public void run() {
        ...
        for (int i = 0; i < joueurs.size();i++){
            joueurCourant.placerRailDebut();
            joueurCourant.enleverNbJetonsRails();
        ...
        List<Joueur> gagnants = gagnants();
        if (gagnants.size()==1){
            log(String.format("%s est le joueur gagnant",gagnants.get(0).toLog()));
        }
        else{
            String s = "Les joueurs gagnants sont : ";
    ...
    public boolean estFini() {
        ...
        int nbrListesVides = 0;
        for (Map.Entry<String, ListeDeCartes> entry : reserve.entrySet()) {
            if (!entry.getKey().equals("Ferraille") && entry.getValue().isEmpty()) {
                nbrListesVides++;
            }
        }
        return nbrListesVides >= 4;
    }

    public List<Joueur> gagnants (){
        int pointsGagnant = joueurs.get(0).getScoreTotal();
        for (Joueur j : joueurs){
            if (j.getScoreTotal()>pointsGagnant){
                pointsGagnant=j.getScoreTotal();
            }
        }
        int finalPointsGagnant = pointsGagnant;
        List<Joueur> gagnants = joueurs.stream().filter(joueur -> joueur.getScoreTotal() == finalPointsGagnant).toList();
        if (gagnants.size()>1){
            int nbRailsGagnant = gagnants.get(0).getNbJetonsRails();
            for (Joueur j : gagnants){
                if (j.getNbJetonsRails() < nbRailsGagnant){
                    nbRailsGagnant = j.getNbJetonsRails();
                }
            }
            int finalNbRailsGagnant = nbRailsGagnant;
            gagnants = gagnants.stream().filter(joueur -> joueur.getNbJetonsRails() == finalNbRailsGagnant).toList();
        }
        return gagnants;
    }
